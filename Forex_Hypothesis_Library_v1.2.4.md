...
**Version:** v1.2.4
**Companion to:** Forex Strategy Validation Playbook v1.0.10
...
**Changelog**
- v1.2.4 — H10 (regime-filtered mean reversion) KILLED at Gate 0, standalone; moved to RESOLVED. EMPTIES THE CANDIDATE PIPELINE — H1-H10 all resolved (all killed), 0 capital risked. Not feasibility/direction (mean reversion is self-directional). Base-rate check on daily majors: faint correctly-signed reversion (VR20 ~0.915; fade-2sd/5d proxy +5.87bps, pooled t ~1.49, 2/9 pairs) = arbitraged residual not a pulse. Durable strand (liquidity-provision premium) is intraday + depth-inverse = near-zero in majors; testable daily strand behavioural w/ speculative counterparty (H6 disease); needs a regime filter that breaks the param budget and is prime curve-fit surface. Filter-grade only. Companion ref → Playbook v1.0.10.
...
| H10 | Regime mean-reversion | 3 | Gate 0 | **Killed** |
...
#### H10 — Regime-filtered mean reversion — KILLED
- **Thesis:** Fade price stretched from a moving average, but *only* in ranging regimes; short-term overreaction corrects. Mean reversion is self-directional (stretched up → short, stretched down → long), so unlike H9 the direction is given by theory. Two strands: a durable liquidity-provision premium (paid for supplying immediacy; Nagel-style, vol-conditional limits-to-arbitrage) and a behavioural overreaction-correction.
- **Reached:** Gate 0 (paper) + a base-rate data check on the box's daily majors (variance ratio, autocorrelation, fade-the-stretch proxy). No strategy backtest.
- **Verdict:** KILL as a standalone. Not feasibility (computable on daily), not direction (theory-given — the one Tier-3 idea where direction isn't the weakness). The data shows a *faint, correctly-signed* reversion (mean VR20 ~0.915 <1; fade->2sd-from-20dMA, 5d-hold proxy +5.87 bps pooled but t only ~1.49, concentrated in 2/9 pairs — USDJPY, USDSEK) — a small mostly-arbitraged residual, not a pulse. Killed on the edge story: the durable strand (liquidity-provision premium) is intraday and scales INVERSELY with market depth, so it is near-zero in FX majors (the deepest market) and untestable without intraday data; the testable daily strand is behavioural overreaction with a speculative/adaptive counterparty (H6 disease); and the rule cannot even be *stated* without a regime filter that blows the <5 free-param budget and is the prime curve-fit surface (lifting a sub-threshold signal by tuning the filter is the forbidden curve-fit). Pre-cost +5.87 bps over 5 days is largely eaten by spread/slippage before a filter is added; the H5 rule applies (positive sign, sub-1 t = kill).
- **Lesson:** A faint correctly-signed empirical pulse is NOT a reason to proceed when the counterparty for the testable strand is speculative and the durable strand doesn't transfer to your universe. Liquidity-provision premia scale inversely with market depth — strongest in illiquid assets, near-zero in FX majors — so an edge real in small-cap equities can be structurally absent in EUR/USD. And an idea that cannot be stated without a regime filter is fragile by construction: the filter, not the signal, becomes the strategy, and the filter is the easiest thing to overfit. Retain as confluence/context (don't fade INTO a fresh stretch; flag mean-reversion context), never standalone. This empties the library — H1-H10 all resolved, all killed, zero capital risked; the flow-based premise held: every idea lacking a forced/locatable/durable counterparty died.

---

## CANDIDATE PIPELINE (untested — full thesis)

*Empty — all ten library hypotheses (H1-H10) are resolved (all KILLED). Use the Persistence Test below to vet any new idea before it earns a slot here.*
