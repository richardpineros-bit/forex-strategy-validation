#!/usr/bin/env python3
"""
oanda_fx_pull_v1.3.0.py — daily FX puller. G10-vs-USD basket for H4
...
exactly with the existing 5. --only PULLS A SUBSET (comma-sep output
names) so frozen files are never touched. Output: fx_data/<PAIR>_daily.csv.
...
def main():
    ...
    ap.add_argument("--only",dest="only",default="",
        help="comma-separated OUTPUT names to pull, e.g. NZDUSD,USDCAD")
    args=ap.parse_args()
    if args.only:
        keep={x.strip().upper() for x in args.only.split(",") if x.strip()}
        for k in list(PAIRS):
            if PAIRS[k] not in keep: del PAIRS[k]
        if not PAIRS: sys.exit("ERROR: --only matched no instruments. Aborting.")
