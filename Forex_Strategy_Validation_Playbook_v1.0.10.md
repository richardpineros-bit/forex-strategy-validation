...
**Changelog**
- v1.0.10 - H10 (regime-filtered mean reversion) KILLED at Gate 0, standalone. EMPTIES THE LIBRARY (H1-H10 all resolved/killed, 0 capital risked). Not feasibility, not direction (mean reversion is self-directional - the one Tier-3 idea where direction is theory-given). Base-rate data check on daily majors: faint correctly-signed reversion (mean VR20 ~0.915 <1; fade->2sd-from-20dMA / 5d-hold proxy +5.87 bps pooled but t ~1.49, concentrated in 2/9 pairs USDJPY/USDSEK) = small mostly-arbitraged residual, not a pulse. Killed on the edge story: durable strand (liquidity-provision premium) is intraday and scales INVERSELY with market depth so near-zero in FX majors and untestable without intraday data; testable daily strand is behavioural overreaction with a speculative/adaptive counterparty (H6 disease); rule cannot be stated without a regime filter that blows the <5 free-param budget and is the prime curve-fit surface. Pre-cost edge eaten by spread; H5 rule (positive sign + sub-1 t = kill). Filter-grade only; retain as confluence, never standalone. No code. Companion library v1.2.4.
...
| 10 | Regime mean-reversion | 3 | Gate 0 | Killed | KILLED standalone. Not feasibility/direction (mean reversion is self-directional). Data base-rate check: faint correctly-signed reversion (VR20 ~0.915; fade-2sd/5d proxy +5.87bps but pooled t ~1.49, only 2/9 pairs) = small arbitraged residual, not a pulse. Durable strand (liquidity-provision premium) is intraday + scales inverse to depth = near-zero in majors, no data; testable daily strand is behavioural w/ speculative counterparty (H6 disease); unstatable without a regime filter that blows <5 param budget + is prime curve-fit surface. H5 rule (sub-1 t = kill). Filter-grade only. No code. |
...
```
I'm running a forex strategy through my validation pipeline (playbook v1.0.10).
